...
# 配置日志：写入当前目录的 mcp_calls.log 文件
logging.basicConfig(
    filename='mcp_calls.log',
    level=logging.INFO,
...
class LLMChatWithMath:
    ...
    def _call_llm_api(self, messages: List[Dict[str, str]], use_tools: bool = False) -> Dict[str, Any]:
        ...
        # 构建系统提示词：强调开放式聊天 + 强制透明计算
        # 核心逻辑：任何涉及数学计算的地方（包括举例、中间推理、验证用户答案）都必须调用工具
        system_prompt = """You are a helpful, conversational AI assistant capable of discussing any topic. 
You have access to a SymPy calculation tool (`sympy_calculate`).

**CRITICAL INSTRUCTION FOR MATH & CALCULATIONS:**
1.  **Transparency & Accuracy**: Whenever you encounter a mathematical expression, calculation, formula evaluation, or need to verify a numerical result (whether explicitly asked by the user OR generated by you as an example/explanation), you **MUST** call the `sympy_calculate` tool first.
2.  **No Mental Math**: Do NOT rely on your internal training data for calculation results (e.g., do not just say "4 * 2 = 8"). Instead, call the tool to get the exact result, then use that result to construct your natural language response.
3.  **Workflow**: 
    - User asks or you plan to give an example involving math -> Call `sympy_calculate`.
    - Receive exact result from tool -> Incorporate the result into your explanation/chat naturally.
    - This applies to simple arithmetic, algebra, calculus, equation solving, and verifying user-provided answers.
4.  **Goal**: Provide rigorous, scientifically accurate calculation results while maintaining a natural, flowing conversation. The tool usage should be seamless to the user.

You can discuss literature, code, daily life, or science. Just remember: if numbers need crunching, use the tool.
"""
    ...
    def _execute_tool(self, tool_call: Dict[str, Any]) -> str:
        ...
        # 记录到日志文件（用户不可见）
        logger.info(f"[MCP CALL] Operation: {operation}, Expression: '{expression}'")
    ...
    def chat(self, user_message: str) -> str:
        ...
        while iteration < self.max_tool_iterations:
            ...
            if tool_calls:
                ...
                logger.info(f"[TOOL REQUEST] LLM requested {len(tool_calls)} tool call(s)")
        ...
        if iteration >= self.max_tool_iterations:
            logger.warning(f"[LIMIT REACHED] Maximum tool iterations ({self.max_tool_iterations}) reached")
